...
merge_hic_with_alu <- function(file_path, output_directory){
  ...
  hic_data[, index := .I]
  big_df$chromo <- sub("\\.csv$", "", big_df$chromo)
  ...
  # Load ATAC-seq data
  atac_data <- read.csv(paste0(file_path, "/atac_data.csv"), header = TRUE)
  atac_data <- atac_data[-(1:18),]  # Remove the first 18 rows
  ...
  write.table(hic_data, paste0(output_directory, "/intermediate.csv"), sep = "\t")
